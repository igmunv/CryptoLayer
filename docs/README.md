...
#### 9. Необязательное поле handshake_timeout

Шаги рукопожатия ждут ответ собеседника не дольше `HANDSHAKE_TIMEOUT` секунд (по умолчанию `60`, значение лежит в `src/config.py`). Если канал медленный - модуль опрашивает мессенджер раз в минуту, сообщения идут с задержкой - минуты может не хватить, и рукопожатие оборвётся по таймауту даже с честным собеседником.

Чтобы этого не происходило, укажите у своего класса поле `handshake_timeout` в секундах:
...
Поле необязательное: если его нет, CryptoLayer возьмёт значение из `src/config.py`, так что уже написанные модули менять не нужно. Пользователь может перекрыть вашу оценку там же, если она не подошла под его условия.

Отдельный лимит `HANDSHAKE_USER_CHECK_TIMEOUT` (по умолчанию `900`) на `handshake_timeout` не смотрит: на том шаге ожидается ручная сверка отпечатка подписи человеком, а не скорость канала.
